...
---
= Digital Advisor 에서 용량 한계에 가까워진 스토리지 시스템을 식별하세요.
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
.단계
. *계획* 위젯에서 *스토리지 확장*을 선택합니다.
+
기본적으로 용량이 90%를 초과했거나 90%에 가까운 ONTAP 시스템이 표시됩니다.

. *StorageGRID* 탭을 선택하면 용량이 70%를 초과했거나 70%에 근접한 StorageGRID 시스템을 볼 수 있습니다.
. 용량을 늘릴 시스템을 선택합니다.
. 향후 6개월간의 용량 예측을 보려면 *용량 예측 보기*를 선택하십시오.
. *용량 증설 요청*을 선택합니다.
. 필요한 경우 설명을 입력합니다.
. 선택한 시스템의 스토리지 확장을 지원받기 위해 NetApp 스토리지 팀에 요청을 보내려면 *보내기*를 선택하십시오.

...
---
= Digital Advisor 통해 보안 위험을 감지하고 시정 조치를 취하세요.
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
. *보안 및 랜섬웨어 방어* 위젯에서 *작업*을 선택합니다. image:Security_Image 2 Ransomware attacks.png["보안 취약점 위젯"]
. 랜섬웨어 방어 * 확인란의 선택을 취소합니다.
. 보안 위험이 높은 경우 운영 체제를 업그레이드하는 권장 조치를 따르십시오.
. *고유 위험* 탭을 선택한 다음 *시정 조치* 열의 링크를 선택하십시오. image:Corrective Action_Image 2 Ransomware attacks.png["수정 조치 열"]
+

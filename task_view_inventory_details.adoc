...
---
= Digital Advisor 에서 스토리지 시스템 인벤토리 세부 정보를 확인하세요.
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
.단계
. *인벤토리* 위젯에서 *시스템*을 선택하면 모든 플랫폼의 시스템 정보를 볼 수 있고, 플랫폼 유형을 선택한 다음 *시스템*을 선택하면 해당 플랫폼에 특화된 시스템 정보를 볼 수 있습니다.
. 시스템에 대한 자세한 정보를 보려면 노드 또는 클러스터를 선택하십시오.
. *다운로드*를 선택하여 *인벤토리* 보고서를 다운로드하고 시스템 세부 정보를 .xls 형식으로 확인하십시오.
. *다운로드*를 선택하여 *Ansible 인벤토리* 보고서를 다운로드하면 지역 또는 사이트 수준에서 .yml 및 .ini 형식으로 시스템 세부 정보를 확인할 수 있습니다.
+
...
.단계
. *인벤토리* 위젯에서 *지원 대상 스위치*를 선택하면 지원 대상 스위치에 대한 모든 정보를 볼 수 있습니다.

...
---
= Digital Advisor 에서 단일 클러스터 및 다중 클러스터용 ONTAP 업그레이드 계획을 생성합니다.
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
업그레이드 계획을 생성하기 전에 다음을 수행해야 합니다.

* AutoSupport(ASUP)이 활성화되어 있고 올바르게 구성되었는지, 그리고 필요한 모든 섹션이 포함되어 있는지 확인하십시오. 자세한 내용은 link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/ONTAP_AutoSupport_Resolution_Guide["ONTAP AutoSupport 해결 가이드"]을 참조하십시오.
* AutoSupport의 클러스터 멤버가 SAP 클러스터 구성과 일치하는지 확인하세요.  자세한 내용은link:https://kb.netapp.com/Support/NSS/Support_Site/How_to_open_a_non-technical_case["비기술적 사례를 열다"] .
...
.단일 클러스터
--
. 대시보드의 *업그레이드 어드바이저* 위젯에서 클러스터 수를 선택합니다.
+
...
. 업그레이드할 클러스터를 선택하십시오. *대상 OS 버전* 열에는 권장 대상 OS 버전이 표시됩니다. image:edit_icon.png["편집 단추 이미지"] 아이콘을 선택하여 클러스터의 다른 대상 OS 버전을 선택할 수 있습니다.
+
image:r_ua_select_target_OS_version_single_cluster.png["대상 버전을 선택하는 방법을 보여 주는 이미지입니다."]

+
** *노드* 열에서 노드 수를 선택하면 클러스터의 노드 요약을 볼 수 있습니다.
** 업그레이드 자격이 없는 클러스터의 문제를 해결하여 업그레이드 자격을 부여하려면 *권장 조치* 열에서 *문제 해결*을 선택할 수 있습니다. + image:r_ua_resolve_issue.png["문제 유형 및 문제 해결에 필요한 해결 방법을 보여 주는 이미지입니다."]


. *업그레이드 계획 생성*을 선택합니다. *업그레이드 권장 사항* 페이지로 이동합니다.
...
** image:ua_export_icon.png["내보내기 버튼"] 아이콘을 선택하면 위험 요약 정보를 Excel 시트로 내보낼 수 있습니다.


. *업그레이드 권장 사항* 페이지에서 *업그레이드 계획 생성*을 선택합니다.
. 표시된 팝업에 세부 정보를 제공합니다.
  를 누릅니다
image:ua_generate_single_clusters_plan.png["업그레이드 계획 생성 요청을 제출하기 위해 세부 정보를 입력하는 팝업."]
. *생성*을 선택합니다. *보고서* 페이지로 이동합니다.
. 업그레이드 계획이 제공되면 * Reports * 페이지에서 다운로드할 수 있습니다.
+

TIP: *업그레이드 어드바이저 보고서 보기*를 선택하면 *보고서* 페이지로 이동합니다. + image:r_ua_view_reports.png["View Upgrade Advisor가 선택을 위해 위치한 곳을 보여주는 스크린샷입니다."]
...
. 업그레이드할 클러스터를 선택합니다. *대상 OS 버전* 열에는 권장 대상 OS 버전이 표시됩니다. image:edit_icon.png["편집 단추 이미지"] 아이콘을 선택하여 클러스터의 다른 대상 OS 버전을 선택할 수 있습니다.
+
image:r_ua_select_target_OS_version.png["대상 버전을 선택하는 방법을 보여 주는 이미지입니다."]

+
** *노드* 열에서 노드 수를 선택하면 클러스터의 노드 요약을 볼 수 있습니다.
** 업그레이드 자격이 없는 클러스터의 문제를 해결하여 업그레이드 자격을 부여하려면 *권장 조치* 열에서 *문제 해결*을 선택하십시오. + image:r_ua_resolve_issue.png["문제 유형 및 문제 해결에 필요한 해결 방법을 보여 주는 이미지입니다."]

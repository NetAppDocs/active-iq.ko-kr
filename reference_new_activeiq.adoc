...
summary: Digital Advisor의 새로운 기능 및 향상된 기능에 대해 알아보십시오. 
---
...
Virtual Machine 기능은 2026년 10월에 Digital Advisor에서 더 이상 사용되지 않을 예정입니다.
...
Value Insights 기능은 ONTAP 시스템의 전반적인 상태, 비용 절감, 안정성 및 보안 정보를 통합하여 제공합니다. 이 정보를 활용하여 ONTAP 환경에 대한 지속적이고 데이터 기반의 관점을 통해 시스템 상태와 성능을 개선할 수 있습니다.
...
* 웰니스 워크플로에 지속가능성 위젯이 추가되었습니다. 이 위젯은 고객, 관심 목록, 사이트 및 그룹 수준에서 권장 조치 횟수를 제공합니다. 지속가능성 대시보드에서 권장 조치에 대한 자세한 보기를 위해 조치 수를 선택할 수 있습니다. 자세한 내용은 link:https://docs.netapp.com/us-en/active-iq/learn-about-sustainability.html["스토리지 시스템의 지속 가능성을 분석합니다"]을 참조하십시오.
...
* *스냅샷 백업 없이 저장* 토글 옵션의 이름이 *스냅샷 복사본 포함*으로 변경되었으며, 기능도 일부 수정되었습니다. link:https://docs.netapp.com/us-en/active-iq/reference_aiq_faq.html#storage-efficiency["자세한 정보"].
...
AFF A-Series, AFF C190, All SAN Array 및 FAS500을 포함한 ONTAP 시스템에서 노드 수준의 SAN 및 NAS 스토리지 효율성을 활용할 수 있습니다.
...
=== NetApp Console과 통합되는 Digital Advisor

Active IQ Digital Advisor가 Digital Advisor로 변경되었으며 이제 하이브리드 멀티클라우드 환경을 위한 NetApp의 통합 관리 콘솔인 NetApp Console에 통합되었습니다. link:https://docs.netapp.com/us-en/active-iq/digital-advisor-integration-with-console.html["자세한 정보"].
...
NetApp Console 사용자는 이제 Digital Advisor의 기존 기능과 유사하게, 해당되는 모든 곳에서 Digital Advisor 링크를 새 탭에서 열 수 있습니다.
...
* 웰니스 리뷰 이메일에는 갱신 정보가 아닌 랜섬웨어 방어에 대한 정보가 포함되어 있습니다.
...
SVM 테이블에서 파란색 'Volume Summary' 상자를 선택하면 특정 일련 번호 또는 물리적 노드에 호스팅되거나 연결된 볼륨에 대한 자세한 정보가 팝업 창으로 표시됩니다.
...
* * 계획 위젯: *
+
** *용량 추가:* 기존 용량의 70% 임계값을 초과하는 GRID 사이트가 있는 경우 알림을 받게 됩니다. 용량 임계값이 70%를 초과할 가능성이 있는 경우, 해당 사이트의 StorageGRID에 대해 향후 1개월, 3개월 또는 6개월 동안 용량을 추가할 수 있는 옵션이 제공됩니다.
** *갱신:* StorageGRID 라이선스 계약이 만료되었거나 향후 6개월 이내에 만료가 임박한 StorageGRID 시스템에 대해 알림이 전송됩니다. 하나 이상의 시스템을 선택하여 NetApp 지원팀에 갱신 요청을 제출할 수 있습니다.
...
* *GRID 뷰어:* *구성* 위젯에서 *GRID 뷰어* 링크를 선택하면 GRID 구성을 자세히 볼 수 있습니다. *구성* 위젯에서 *Grid Viewer* 화면의 *다운로드* 버튼을 선택하면 선택한 StorageGRID의 사이트 세부 정보 및 용량 세부 정보를 다운로드할 수 있습니다.
* * 사이트 세부 정보: * 이 탭은 각 사이트에 사용할 수 있는 그리드 요약 및 스토리지 노드를 제공합니다.
* *GRID 요약:* 라이선스 유형, 라이선스 용량, 설치된 노드 수, 지원 기간(라이선스 계약 종료일), 기본 관리 노드 및 기본 관리 노드의 기본 사이트 등 기본 정보를 포함합니다. 이 탭에는 사이트 이름과 해당 사이트에 태그된 스토리지 노드 수도 표시됩니다. 이번 릴리스에서는 해당 사이트의 스토리지 노드를 보기 위해 제공된 하이퍼링크를 선택하면 노드 이름 목록을 확인할 수 있습니다.
...
Tier Data 버튼이 FabricPool 대시보드에 추가되었으며, 이 버튼을 사용하면 NetApp Console을 통해 저렴한 오브젝트 스토리지 계층으로 데이터를 계층화할 수 있습니다.
...
Digital Advisor API를 사용하면 관심 있는 데이터를 가져와 회사 워크플로에 직접 통합할 수 있습니다. link:https://docs.netapp.com/us-en/active-iq/concept_overview_API_service.html["자세한 정보"].
...
AFF 시스템, 비AFF 시스템 또는 둘 다에 대해 스냅샷 복사본 사용 여부에 따른 스토리지 시스템의 스토리지 효율성 비율과 절감 효과를 확인할 수 있습니다. 스토리지 효율성 정보는 노드 수준에서 확인할 수 있습니다. link:https://docs.netapp.com/us-en/active-iq/task_analyze_storage_efficiency.html["자세히 알아보기"].

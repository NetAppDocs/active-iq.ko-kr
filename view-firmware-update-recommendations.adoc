...
summary: 스토리지 펌웨어, SP/BMC 펌웨어, 시스템 파일 및 보안 파일의 업데이트 권장 사항을 확인합니다. 
---
= Digital Advisor 에서 펌웨어 업데이트 권장 사항을 확인하세요.
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
.단계
. 대시보드 페이지에서 *Upgrade Advisor* 위젯의 *Other recommendations*를 선택합니다.
. 추천 목록을 보려면 *추천 보기*를 선택합니다.
. *업데이트 상태* 열에서 *진행 상황 보기*를 선택하면 업데이트 가능한 클러스터를 볼 수 있습니다.
. 문제가 발생할 경우, 해당 링크를 선택하여 문제 완화 단계를 확인하십시오.

...
---
= Digital Advisor 의 클라우드 권장 사항을 통해 스토리지 시스템 상태를 분석하세요.
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
NOTE: Digital Advisor 권장 사항을 구현하기 위해 NetApp 콘솔로 안내합니다.
...
다음 기준을 충족하는 볼륨은 Cloud Volumes ONTAP (CVO)으로 마이그레이션하는 것이 좋습니다.
...
.단계
. 왼쪽 창에서 *클라우드 추천*을 선택합니다.
. *마이그레이션* 창에서 링크 하나를 선택합니다.
. *클라우드로 마이그레이션*을 선택하여 *콘솔*을 실행합니다.
...
.단계
. 왼쪽 창에서 *클라우드 추천*을 선택합니다.
. * Tiering * 창에서 링크를 하나 선택합니다.
. *티어 데이터*를 선택하여 *콘솔*을 실행합니다.
...
.단계
. 왼쪽 창에서 *클라우드 추천*을 선택합니다.
. *백업 및 보관* 창에서 링크를 하나 선택합니다.
. *클라우드 백업*을 선택하여 *콘솔*을 실행합니다.
...
.단계
. 왼쪽 창에서 *클라우드 추천*을 선택합니다.
. *재해 복구* 창에서 링크 하나를 선택하십시오.
. *클라우드에 복제*를 선택하여 *콘솔*을 실행합니다.
